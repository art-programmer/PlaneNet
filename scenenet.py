...
class SceneNet(Network):
    def setup(self, is_training, options):
        ...
        output_stride = options.outputStride
        ...
        if output_stride == 8:
            ...
            (self.feed('res5c')
             .relu(name='res5c_relu_8')
             .max_pool(2, 2, 2, 2, name='res5c_relu'))
            pass


        (self.feed('res5c_relu')
             .avg_pool(HEIGHT / output_stride, WIDTH / output_stride, HEIGHT / output_stride, WIDTH / output_stride, name='res5d_pool1')
             ...
             .resize_bilinear(size=[HEIGHT / output_stride, WIDTH / output_stride], name='res5d_upsample1'))

        (self.feed('res5c_relu')
             .avg_pool(HEIGHT / output_stride / 2, WIDTH / output_stride / 2, HEIGHT / output_stride / 2, WIDTH / output_stride / 2, name='res5d_pool2')
             ...
             .resize_bilinear(size=[HEIGHT / output_stride, WIDTH / output_stride], name='res5d_upsample2'))

        (self.feed('res5c_relu')
             .avg_pool(HEIGHT / output_stride / 4, WIDTH / output_stride / 4, HEIGHT / output_stride / 4, WIDTH / output_stride / 4, name='res5d_pool3')
             ...
             .resize_bilinear(size=[HEIGHT / output_stride, WIDTH / output_stride], name='res5d_upsample3'))

        (self.feed('res5c_relu')
             .avg_pool(HEIGHT / output_stride / 8, WIDTH / output_stride / 8, HEIGHT / output_stride / 8, WIDTH / output_stride / 8, name='res5d_pool4')
             ...
             .resize_bilinear(size=[HEIGHT / output_stride, WIDTH / output_stride], name='res5d_upsample4'))
            ...
            with tf.variable_scope('deep_supervision'):
                for layerIndex, layer in enumerate(options.deepSupervisionLayers):
                    ...
                    (self.feed(layer+'_pool1')
                         .reshape(shape=[-1, nChannels_4], name=layer+'_object_reshape1')
                         .fc(num_out=options.numOutputObjects * 12, name=layer+'_object_fc', relu=False)
                         .reshape(shape=[-1, options.numOutputObjects, 12], name=layer+'_object_pred'))

                    if options.predictConfidence == 1:
                        (self.feed(layer+'_object_reshape1')
                             .fc(num_out=options.numOutputObjects, name=layer+'_object_confidence_fc', relu=False)
                             .reshape(shape=[-1, options.numOutputObjects, 1], name=layer+'_object_confidence_pred'))
                        pass
                    continue
                pass
            pass
        ...
        (self.feed('res5d_pool1')
             .reshape(shape=[-1, nChannels_5], name='object_reshape1')
             .fc(num_out=options.numOutputObjects * 12, name='object_fc', relu=False)
             .reshape(shape=[-1, options.numOutputObjects, 12], name='object_pred'))

        if options.predictConfidence == 1:
            (self.feed('object_reshape1')
                 .fc(num_out=options.numOutputObjects, name='object_confidence_fc', relu=False)
                 .reshape(shape=[-1, options.numOutputObjects, 1], name='object_confidence_pred'))
            pass
        ...
        if options.predictLocal == 1:
            ...
            (self.feed('segmentation_dropout')
                 .conv(1, 1, 12, 1, 1, relu=False, name='local_object_pred'))
            (self.feed('segmentation_dropout')
                 .conv(1, 1, options.numClasses, 1, 1, relu=False, name='local_class_pred'))
            pass
